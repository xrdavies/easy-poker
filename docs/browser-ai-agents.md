...
5. 未坐下的 AI 自动执行 `sit`；因此连续三次超时被强制起身后，刷新即可重新加入。
...
- 模型服务必须允许浏览器跨域请求。模型配置应提供“测试连接”，明确区分 CORS、认证和模型不存在错误。
...
- 从 `scripts/agent.mjs` 提取可在 Node 和浏览器复用的 Prompt、`apiUrl`、`legalMove` 和模型决策函数。
