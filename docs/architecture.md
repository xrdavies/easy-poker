...
牌组、摊牌比较、边池、街机状态、买入、随机空位、straddle / 鱿鱼 / 27 杂色、10 秒行动钟（注入 `now()`）都在引擎里，不依赖 Worker 或 DOM。27 杂色仅在翻牌后逼退所有对手时触发并自动亮牌，其他在座玩家各支付 5BB；多人摊牌不奖励。补码记入 pending，下一手 `startHand` 时到账。两人 All-in 且公共牌未发完时进入 5 秒发牌协商（默认发一次，双方都选发两次才跑两次）。超时默认过牌，不能过则弃牌并写入 lastResult 展示结算。
...
锦标赛、账户体系、大厅匹配、把 UI 放到 Cloudflare Pages。
