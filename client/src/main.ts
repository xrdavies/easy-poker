import {
  Engine,
  Renderer2D,
  UIBridge,
} from "@xrdavies/2d-engine";
import { PokerNet } from "./net.ts";
import { startOverlay } from "./overlay.ts";
import { PokerSfx } from "./sfx.ts";
import { syncTableEngine, tableScreenIsVisible } from "./table-engine.ts";
import { TableSurface } from "./table-surface.ts";

export { Engine, Renderer2D, UIBridge };
export { SEATS, inviteUrl, isPortraitTable, renderShowdown, actionMsLeft } from "./overlay.ts";
export { SFX_IDS, SFX_POOL } from "./sfx.ts";
export { syncTableEngine, tableScreenIsVisible } from "./table-engine.ts";

declare global {
  interface Window {
    __easyPokerEngine?: Engine | null;
    __EASY_POKER_USES_2D_ENGINE?: boolean;
  }
}

window.__EASY_POKER_USES_2D_ENGINE = true;

/** Table-canvas overlay only. Never pass #gate / #settle-screen / page chrome. */
export function attachTableUiBridge(canvas: HTMLCanvasElement): UIBridge {
  return new UIBridge(canvas);
}

async function boot(): Promise<void> {
  const canvas = document.querySelector<HTMLCanvasElement>("#game-canvas");
  if (!canvas) throw new Error("Easy Poker shell is incomplete");

  const sfx = new PokerSfx();
  const net = new PokerNet();
  void sfx.preload();

  const bridge = attachTableUiBridge(canvas);
  bridge.setInputCaptured(false);

  startOverlay({ net, sfx });
  document.body.addEventListener("pointerdown", () => void sfx.unlock(), { once: true });

  const table = document.getElementById("table-screen");
  let engine: Engine | null = null;
  let starting = false;

  const startEngine = (): void => {
    if (starting || engine) return;
    starting = true;
    void (async () => {
      try {
        const created = await Engine.create({ canvas, autoStart: true, input: true });
        const renderer = new Renderer2D(created.gpu, {
          clearColor: { r: 0.027, g: 0.063, b: 0.094, a: 1 },
        });
        created.addSystem(new TableSurface(created, renderer));
        engine = created;
        window.__easyPokerEngine = created;
        document.body.classList.add("engine-gpu");
        if (!tableScreenIsVisible(table)) created.pause();
        else {
          created.resize();
          bridge.sync();
        }
      } catch (error) {
        starting = false;
        window.__easyPokerEngine = null;
        document.body.classList.add("engine-fallback");
        console.warn("2d-engine WebGPU init failed; overlay chrome still runs", error);
      }
    })();
  };

  const apply = (): void =>
    syncTableEngine(tableScreenIsVisible(table), engine, startEngine);
  if (table) {
    new MutationObserver(apply).observe(table, { attributes: true, attributeFilter: ["class"] });
  }
  apply();
}

void boot();
